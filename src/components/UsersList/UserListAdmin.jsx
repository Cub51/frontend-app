...
const Users = () => {
    const { users,
        updateUser,
        deleteUser,
        getUsersAdmin } = useContext(UserContext);
    const [showModal, setShowModal] = useState(false);
    const [showModalContra, setShowModalContra] = useState(false);
    const [selectedUser, setSelectedUser] = useState(null);
    const [updatedUser, setUpdatedUser] = useState({ _id: '', nombre: '', apellido: '', correo: '', contrasena: '' });
    ...
    const handleContraChange = (e) => {
        ...
        setUpdatedUser({ ...updatedUser, _id: selectedUser._id, [e.target.name]: e.target.value });
    };
    // usuario
    const handleShowModal = (user) => {
        setSelectedUser(user);
        setFormData({ nombre: user.nombre, apellido: user.apellido, correo: user.correo });
        setShowModal(true);
    };
    ...
    const handleSave = () => {
        ...
        setShowAlert(true);
        setUpdatedUser({ _id: '', nombre: '', apellido: '', correo: '', contrasena: '' });
    ...
    return (
        <div className={styles.container_curs__list}>
             <h2 className="my-4">Lista de Administradores</h2>
            <Container>
                <Row>
                    <Col key={1} sm={12} md={6} lg={4} className="mb-4" >
                        <Card style={{ textAlign: 'center' }}>
                            <Card.Body>
                                <Card.Title >
                                    <strong>  Agregar usuario Administrador </strong>
                                </Card.Title>
                                <Card.Text>
                                    ...
                                    <Card.Body>
                                        ...
                                        <Card.Text>
                                            <div style={{ color: "gray" }}>
                                                ...
                                                <br />
                                                <strong>Fecha de Registro:</strong> {new Date(user.fechaRegistro).toLocaleDateString('es-ES', { day: 'numeric', month: 'long', year: 'numeric' })}
                                                <br />
                                                <strong>Ultima Actualización:</strong> {new Date(user.fechaActualizacion).toLocaleDateString('es-ES', { day: 'numeric', month: 'long', year: 'numeric' })}
                                                <br />
                                            </div>
                                            <strong > Correo: </strong>  {user.correo}
                                            <br />
                                        </Card.Text>
                                        <Button variant="primary" onClick={() => { handleShowModal(user), setShowAlert(true) }} className="me-2">
                                            Actualizar
                                        </Button>
                                        <Button variant="secondary" onClick={() => { handleShowModalContrasena(user), setShowAlert(true) }} className="me-2">
                                            Contraseña
                                        </Button>
                                        <Button variant="danger" onClick={() => { handleDeleteUser(user._id) }} className="">
                                            Borrar
                                        </Button>
                                    </Card.Body>
                                </Card>
            ...
            <Modal show={showModal} onHide={handleCloseModal}>
                ...
                <Modal.Footer>
                    ...
                    <Button variant="primary" onClick={() => { handleSave(), showAlert && (mostrarAlerta('Usuario actualizado correctamente.', 'alert-success'), setShowAlert(false)) }}>
                        Guardar
                    </Button>
                </Modal.Footer>
            </Modal>
